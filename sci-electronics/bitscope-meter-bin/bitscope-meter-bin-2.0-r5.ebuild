...
src_install() {
	...
	sed -i 's/\/usr\/bin\/bitscope-meter/\/opt\/bitscope-meter-bin-2.0\/usr\/bin\/bitscope-meter/g' \
	"${D}"/opt/"${P}"/usr/share/applications/bitscope-meter.desktop
	# remove the icon file extension
	sed -i 's/.png//g' "${D}"/opt/"${P}"/usr/share/applications/bitscope-meter.desktop
	# remove the deprecated value Application
	sed -i 's/;Application//g' "${D}"/opt/"${P}"/usr/share/applications/bitscope-meter.desktop
