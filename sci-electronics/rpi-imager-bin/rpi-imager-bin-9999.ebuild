...
RESTRICT="strip"  # prevents stripping the binary, rpi-imager will not run when stripped
...
src_prepare(){
	...
	sed -i 's/\/usr\/bin\/rpi-imager/\/opt\/rpi-imager-bin-9999\/usr\/bin\/rpi-imager/g' \
	                           "${desktop_file}"
}
